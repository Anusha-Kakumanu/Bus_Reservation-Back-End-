...
public class CustomerDao extends GenericDao {
	...
	// if there's no customer with the given combination, this version will throw an
	// exception
	public int isValidUserV2(String email, String password) {
		return (Integer) entityManager.createQuery("select c.id from Customer c where c.email= :em and c.password= :pw")
				.setParameter("em", email)
